...
func (w *writer) createErc20Proposal(m msg.Message) bool {
	...
	w.log.Info("Proposal Amount", "src", new(big.Int).SetBytes(m.Payload[0].([]byte)).String())
	w.log.Info("Proposal Destination Address", "src", common.BytesToAddress(m.Payload[1].([]byte)))
	w.log.Info("Proposal dataHash", "src", dataHash)
